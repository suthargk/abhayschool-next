...
import { TestimonialDeleteDialog } from "./testimonial-table/delete-dialog";
...
export function TestimonialAdmin({ initialItems, canPublish }) {
  ...
  async function togglePublish(item) {
    ...
    try {
      await fetch(`/api/super-admin/testimonials/${item.id}/publish`, {
        method: "POST",
    ...
    try {
      await Promise.all(
        deleteTarget.ids.map((id) =>
          fetch(`/api/super-admin/testimonials/${id}`, { method: "DELETE" }),
        ),
      );
  ...
  async function reorder(orderedIds) {
    ...
    await fetch("/api/super-admin/testimonials/reorder", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },

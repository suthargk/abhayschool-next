...
import { RoutePlanRow } from "./row";

export function RoutePlanTable({
  items,
  canPublish,
  pendingId,
  onReorder,
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter(
      (item) =>
        item.place.toLowerCase().includes(q) || String(item.routeNo).includes(q),
    );
  }, [items, search]);
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative max-w-sm flex-1">
          ...
          <Input
            ...
            placeholder="Search by place or route no..."
            className="pl-8"
          />
        </div>

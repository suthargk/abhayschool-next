...
export function TopperForm({ initialItem, canPublish }) {
  ...
  async function handlePhotoChange(e) {
    ...
    try {
      ...
      const res = await fetch("/api/super-admin/toppers/upload", {
        method: "POST",
        body: formData,
      });
  ...
  async function handleSubmit(e) {
    ...
    try {
      const res = await fetch(
        isEdit ? `/api/super-admin/toppers/${initialItem.id}` : "/api/super-admin/toppers",
        {
      ...
      router.push("/super-admin/achievements/toppers");
      router.refresh();
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  async function handleTogglePublish() {
    ...
    setError("");
    try {
      const res = await fetch(`/api/super-admin/toppers/${initialItem.id}/publish`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <form onSubmit={handleSubmit} className="max-w-3xl space-y-6">
      ...
      <div className="flex gap-2">
        ...
        <Button
          type="button"
          variant="outline"
          onClick={() => router.push("/super-admin/achievements/toppers")}
        >
          Cancel
        </Button>
      </div>
